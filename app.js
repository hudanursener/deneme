...
const url = "mongodb+srv://hudanur:1453Telefon@cluster0-tqiss.mongodb.net/test";
const dbName = "TelefonVeriTabani1";

app.get("/cihaz", function(req, res) {
  try {
    rica.post(
      anaDizin + "?token=" + apiKey + "&device=" + req.query.cihazAdi,
      function(err, cevap, govde) {
...
app.get("/ekle", function(req, res) {
  try {
    rica.post(
      anaDizin + "?token=" + apiKey + "&device=" + req.query.EklemeYap,
      function(err, cevap, govde) {
        var veri = JSON.parse(govde);
      }
    );
